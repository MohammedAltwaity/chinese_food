...
------------------------------------------------
Uses local Haar cascade XML file for face extraction.
"""
...
def extract_face_with_rotation(image, margin=DEFAULT_MARGIN, fallback_angles=FALLBACK_ANGLES):
    """
    Extracts face using local Haar cascade in the script root.
    """
    haar_path = "haarcascade_frontalface_default.xml"  # local file
    if not os.path.exists(haar_path):
        print("❌ Haar cascade XML not found in current directory")
        return image
...
def index_route():
    if request.method == "POST":
        ...
        for i, f in enumerate(frames_sorted):
            face_img = extract_face_with_rotation(f)
